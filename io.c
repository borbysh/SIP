...
void read_field(field *temperature, char *filename)
{
    FILE *fp;
    int nx, ny, i, j;
    int count;
    ...
    set_field_dimensions(temperature, nx, ny);
    

    /* Allocate arrays (including ghost layers */
    temperature->data = malloc_2d(nx + 2, ny + 2);
    

    /* Read the actual data */
    for (i = 1; i < nx + 1; i++) {
        for (j = 1; j < ny + 1; j++) {
            fscanf(fp, "%lf", &temperature->data[i][j]);
        }
    }

    /* Set the boundary values */
    for (i = 1; i < nx + 1; i++) {
        temperature->data[i][0] = temperature->data[i][1];
        temperature->data[i][ny + 1] = temperature->data[i][ny];
    }
    for (j = 0; j < ny + 2; j++) {
        temperature->data[0][j] = temperature->data[1][j];
        temperature->data[nx + 1][j] = temperature->data[nx][j];
    }
